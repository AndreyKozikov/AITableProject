...
# ==================== МАППИНГ ДАННЫХ ====================
# Используется в: mapper.py, ask_qwen3_cot.py
...
# Флаг: загружать ли предобученные параметры (LoRA адаптеры)
# Меняется напрямую здесь, без переменных окружения
LOAD_LORA_ADAPTERS = False

# Размер чанка записей для маппера (кол-во записей в одном чанке)
MAPPING_CHUNK_SIZE = 1
...
PROMPT_TEMPLATE_SO = """
Ты — ассистент по строгой структурированной обработке табличных данных.  
Твоя задача — извлекать сведения из входных данных и распределять их по указанным столбцам строго в формате JSON.

Обязательные правила:

1. Используй ровно такие ключи: {header}.  
2. Структура вывода должна полностью соответствовать JSON Schema (см. ниже).  
3. Не создавай дополнительные ключи, поля, массивы или уровни вложенности.  
4. Если данных нет — заполняй значением пустой строки.  
5. Выводи только корректный JSON без пояснений, без форматирования в стиле Markdown.  
6. Не допускается перенос одного и того же значения в разные столбцы.  
   - Если значение уже помещено в один столбец, другие столбцы не могут содержать его полностью или частично.  
   - Если входные данные допускают неоднозначность — выбирай наиболее подходящий столбец и оставляй остальные пустыми.  
7. Не переформулируй значения, не объединяй их и не дели. Используй исходный текст без преобразований.  

<output-format>
Строго следуй следующей JSON-схеме:
{schema}
</output-format>
