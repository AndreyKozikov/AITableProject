...
def estimate_tokens(csv_text: str, question: str = "Анализируй таблицу") -> dict:
    """
...
def max_rows_for_csv(image_path: str, df: pd.DataFrame, question: str = "Анализируй таблицу", avg_tokens_per_row: int = 25) -> int:
    ...
    dummy = estimate_tokens("", question)
...
def estimate_quantity_tokens(df):
    rows = max_rows_for_csv(df)
