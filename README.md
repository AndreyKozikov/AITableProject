...
> 📚 **Навигация по документации**: Полный индекс всех документов проекта доступен в **[DOCUMENTATION_INDEX.md](DOCUMENTATION_INDEX.md)**
>
> 🚀 **Быстрый старт**: Для быстрого запуска проекта смотрите **[QUICKSTART.md](QUICKSTART.md)**
>
> 📊 **Для руководства**: Executive Summary доступен в **[EXECUTIVE_SUMMARY.md](EXECUTIVE_SUMMARY.md)**
...
- Аналитики данных, работающие с документами
- Компании, обрабатывающие отсканированные документы с табличными данными
- Исследователи, работающие с PDF и изображениями таблиц
- Команды управления документами
...
├── parsing_files/            # Промежуточные данные парсинга
├── logs/                     # Логи приложения
├── model_table/              # Справочные таблицы моделей
...
- **`df_utils.py`** — утилиты для работы с DataFrame
  - Очистка данных
  - Экспорт в JSON/Excel
  - Реконструкция таблиц из OCR-результатов
...
### 5. **Структурированный вывод (Structured Output)**
...
### Выходные форматы
- **Excel** (`.xlsx`) — основной формат результатов
- **JSON** — промежуточные структурированные данные
- **CSV** — экспорт таблиц (опционально)
...
1. **Использование GPU**
   - Установите `paddlepaddle-gpu` и `torch` с CUDA
   - Ускорение обработки в 3-10 раз

2. **Предобработка изображений**
   - Настройте параметры в `config/preprocess_image_config.yaml`
   - Отключите избыточные фильтры для ускорения

3. **Выбор модели**
   - **Qwen 3 локально**: быстрее, конфиденциально, требует ресурсов
   - **OpenAI API**: медленнее (сеть), высокая точность, без требований к железу

4. **Batch обработка**
   - Обрабатывайте несколько файлов за раз через веб-интерфейс
