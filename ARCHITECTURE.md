...
└────────────┬────────────────────┘
   ...
   (Excel файлы для обучения)
             ...
             ▼
┌─────────────────────────────────┐
│  combine_excel_data_2.py        │
│  1. Чтение Excel из datasets/   │
│     data/                       │
│  2. Объединение всех строк      │
│  3. Случайная выборка (500+)    │
│  4. Аугментация данных:         │
│     - Размерные характеристики  │
│     - Описательные параметры    │
│  5. Создание output.xlsx с      │
│     тремя листами               │
└────────────┬────────────────────┘
             │
             ▼
...
│  2. Формирование JSONL          │
│  3. Создание промптов с JSON    │
│     схемой (Pydantic)           │
│  4. Генерация эталонных ответов │
└────────────┬────────────────────┘
             │
             ▼
   qwen3_structured_train.jsonl
   (Готовый датасет для обучения)
```
...
┌─────────────────────────────────┐
│  finetune_qwen3.py              │
│  1. Загрузка модели из          │
│     models/qwen/ (локально)     │
│  2. Настройка LoRA адаптеров    │
│     - r=16, alpha=32            │
│     - target: q_proj, v_proj    │
│  3. Токенизация датасета        │
│  4. Обучение (3 эпохи)          │
│  5. Сохранение адаптеров        │
└────────────┬────────────────────┘
             │
             ▼
     models/qwen/
     ├── checkpoints/          (промежуточные)
     │   ├── checkpoint-xxx/
     │   └── logs/
     └── lora_adapters/        (финальные)
